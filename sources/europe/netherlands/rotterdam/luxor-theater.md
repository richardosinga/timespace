---
title: Luxor Theater Rotterdam
poi: spacetime/europe/netherlands/rotterdam/luxor-theater
url: https://www.luxortheater.nl/agenda
interval_days: 7
last_checked: 2026-06-05
---
Major theatre near Rotterdam Centraal with a varied programme of musical
theatre, dance, stand-up comedy and large-scale international productions.

Look for: premieres, first nights, special performances and limited-run
shows in the next 60 days. Use category `theatre`.

This venue has a shadow POI: `spacetime/europe/netherlands/rotterdam/luxor-theater`.
